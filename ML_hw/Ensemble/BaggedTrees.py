...
def bagging(X, attributes, Y, tree_num=100, sample=True):
    trees = []
    for i in range(tree_num):
        if sample:
            sample_x, sample_y = subsample(X,Y)
            tree = DecisionTreeClassifier(S=sample_x, attributes=attributes, labels=sample_y, max_depth=-1)
        else:
            tree = DecisionTreeClassifier(S=X, attributes=attributes, labels=Y, max_depth=-1)
        tree.id3()
        trees.append(tree)
    return trees
